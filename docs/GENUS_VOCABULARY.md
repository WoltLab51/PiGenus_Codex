...
| WorkerSchedulingPreview | implemented | Storage-free explanation of which known workers could host a requested capability under simple constraints. | Uses WorkerInspectionService and produces suitable/unsuitable candidates with reasons; can convert to a log-compatible GovernanceDecision. | `pigenus.core.worker_scheduling_preview.WorkerSchedulingPreviewService`. | Not scheduling, reservation, persistence, routing, provider calls, or execution. |
