...
| WorkerExecutionPreflightCLI | implemented | Read-only command-line surface for one-worker execution eligibility checks. | Reads WorkerStore records, builds temporary in-memory preflight state, and prints ordered checks. | `worker-execution-preflight`. | Not logging, scheduling, assignment, reservation, routing, provider calls, or execution. |
