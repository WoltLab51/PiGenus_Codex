...
| WorkerSchedulingPreviewCLI | implemented | Read-only command-line surface for previewing worker suitability. | Reads WorkerStore records, builds temporary in-memory preview state, and prints decision/reason/candidate rows. | `worker-scheduling-preview`. | Not logging, scheduling, assignment, reservation, routing, provider calls, or execution. |
