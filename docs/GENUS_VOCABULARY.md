...
| WorkerProfile | documented | Future structured record describing worker identity, runtime inventory, cost, privacy, and status surfaces. | Prepared by Worker Runtime readiness before WorkerRegistry implementation. | `docs/WORKER_RUNTIME_READINESS.md`. | Not a scheduler and not proof of authorization. |
| WorkerHeartbeat | documented | Future liveness and freshness signal for a worker. | Helps decide whether a known worker can be considered for governed execution later. | `docs/WORKER_RUNTIME_READINESS.md`. | Not health proof by itself and not permission to execute. |
