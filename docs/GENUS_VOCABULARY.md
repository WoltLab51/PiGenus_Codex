...
| WorkerSchedulingPreview | implemented | Storage-free explanation of which known workers could host a requested capability under simple constraints. | Uses WorkerInspectionService and produces suitable/unsuitable candidates with reasons. | `pigenus.core.worker_scheduling_preview.WorkerSchedulingPreviewService`. | Not scheduling, reservation, routing, provider calls, or execution. |
