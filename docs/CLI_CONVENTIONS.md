...
`worker-execution-preflight` is read-only and does not expose `--log` yet.
...
`worker-execution-preflight`, `context-boundary-check` without `--log`,
`context-boundary-list`, and `permission-list` are read-only.
